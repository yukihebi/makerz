...
/// directory regardless of the shell's cwd. Each `(key, value)` in
/// `env_entries` is emitted as `--env KEY=VALUE`, preserving order.
/// [`OsString`] preserves non-UTF-8 names losslessly on Unix.
///
/// A `--cwd` on the passthrough side is left alone; if the user supplied one,
/// `makers` itself decides which wins.
pub fn build_args(
    makefile_dir: &Path,
    env_entries: &[(String, OsString)],
    passthrough: &[String],
) -> Vec<OsString> {
    ...
    for (key, value) in env_entries {
        ...
        args.push(env_kv(key, value));
    }
    args.extend(passthrough.iter().map(OsString::from));
    args
}

fn env_kv(key: &str, value: &OsString) -> OsString {
